...
public class SmartPool : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        InvokeRepeating(nameof(StartSpawningZombies), 1f, Random.Range(1f, 5f));
    }
    ...
    public void SpawnBullet(Vector3 position, Vector3 direction, Quaternion rotation, NameWeapon nameWeapon)
    {
        if (nameWeapon != NameWeapon.ROCKET)
        {
            for (int i = 0; i < bullet_Prefabs.Count; i++)
            {
                if (!bullet_Prefabs[i].activeInHierarchy)
                {
                    ...
                    // SET BULLET DAMAGE
                    SetBulletDamage(nameWeapon, bullet_Prefabs[i]);
        ...
        else
        {
            for (int i = 0; i < rocket_Bullet_Prefabs.Count; i++)
            {
                if (!rocket_Bullet_Prefabs[i].activeInHierarchy)
                {
                    ...
                    //SET BULLET DAMAGE
                    SetBulletDamage(nameWeapon, rocket_Bullet_Prefabs[i]);
    ...
    void SetBulletDamage(NameWeapon nameWeapon, GameObject bullet)
    {
        switch (nameWeapon)
        {
    ...
    void StartSpawningZombies()
    {
        ...
        {
            CancelInvoke(nameof(StartSpawningZombies));
        }
    }
}
